...
export async function createProfissionaisLote(context: IExecuteFunctions, index: number): Promise<any> {
    ...
    try {
        ...
        throw new NodeOperationError(
            context.getNode(),
            'O campo "Profissionais para Cadastro (JSON)" para cadastro deve conter um array JSON válido.',
            {itemIndex: index},
        );
    }
    ...
    if (profissionais.length === 0) {
        throw new NodeOperationError(
            context.getNode(),
            'O campo "Profissionais para Cadastro (JSON)" deve ter pelo menos um registro.',
            {itemIndex: index},
        );
    }
    try {
