...
export async function editProfissionaisLote(context: IExecuteFunctions, index: number): Promise<any> {
    const profissionaisJson = context.getNodeParameter('profissionais_edicao_json', index);
    ...
    try {
        ...
        throw new NodeOperationError(
            context.getNode(),
            'O campo "Profissionais para Edição (JSON)" para cadastro deve conter um array JSON válido.',
            {itemIndex: index},
        );
    }
    ...
    if (profissionais.length === 0) {
        throw new NodeOperationError(
            context.getNode(),
            'O campo "Profissionais para Edição (JSON)" deve ter pelo menos um registro.',
            {itemIndex: index},
        );
    }
    try {
