...
export async function enturmarEstudantesLote(
    ...
    try {
        ...
        throw new NodeOperationError(
            context.getNode(),
            'O campo "Matrículas para Enturmação (JSON)" para cadastro deve conter um array JSON válido.',
            {itemIndex: index},
        );
    }
    ...
    if (matriculas.length === 0) {
        throw new NodeOperationError(
            context.getNode(),
            'O campo "Matrículas para Enturmação (JSON)" deve ter pelo menos um registro.',
            {itemIndex: index},
        );
    }
